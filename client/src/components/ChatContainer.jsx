...
const ChatContainer = ({ logout }) => {
    ...
    const [typingUser, setTypingUser] = useState({}); // Track typing user data
    ...
    useEffect(() => {
        ...
        socketio.on('typing', ({ user, avatar, status }) => {
            console.log(`${user} is typing...`);
            setTypingUser({ user, avatar, status });
            setIsTyping(true);
            setTimeout(() => {
                setIsTyping(false);
                setTypingUser({});
        ...
        <div className="flex items-start justify-center bg-[#fef9fa] border-t ">
            ...
            <div className="sm:w-3/4 w-full border-l-1 ">
                ...
                <div className="flex-grow overflow-y-auto border-t border-b h-[calc(100vh-13rem)] hide-scrollbar " ref={chatListRef}>
                    <ChatsLists />
                    {/* Display typing indicator with avatar and status */}
                    {isTyping && (
                        <ChatBoxReciever message='...' avatar={typingUser.avatar} color={typingUser.status} user={typingUser.user} />

                    )}
                </div>
                <div className="z-10 flex items-center justify-center mt-auto ">
                    <InputText addMessage={addMessage} handleTyping={() => socketio.emit('typing', { user, avatar, status: localStorage.getItem('status') || 'success' })} />
                </div>
            </div>
        </div>
    );
